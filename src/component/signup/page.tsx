...
import { Button, CheckMark, Eye, EyeSlash, Input, XMark } from "../common";
          ...
          {!!formData.password.value.length && (
            <div>
              <PasswordValidationItem
                isValid={
                  formData.password.value.length >= 5 &&
                  /\d/.test(formData.password.value) &&
                  /[a-z]/.test(formData.password.value) &&
                  /[A-Z]/.test(formData.password.value)
                }
                text="Password must be at least 5 characters long, contain at least one uppercase letter, one lowercase letter, and one digit."
              />
            </div>
          )}
        </form>
...
const PasswordValidationItem = ({ isValid, text }) => (
  ...
  >
    {isValid ? (
      ...
      <XMark className="text-red-[#cd00] cursor-none" />
    )}{" "}
    <div>{text}</div>
  </div>
);
